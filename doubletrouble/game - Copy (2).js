// EchoChase.io Game with Two Worlds (Prototype)
...
function handleInput(player, index) {
  if (index === 0) {
    ...
    if (keys["Shift"]) {
      // Reverse direction for both players
      player.dx = -player.dx;
      player.dy = -player.dy;
    }
  } else {
    // Collecting player mirrors the dodging player in opposite directions unless modifiers are pressed
...
function checkCollisions() {
  ...
  coins = coins.filter((coin) => {
    ...
    if (dist < playerRadius + coinRadius) {
      goodSound.play();
      score++;
      speedBoostP2 += speedIncreaseRateP2; // Increase P2's speed for each collected coin
      cannonInterval = Math.max(10, cannonInterval - cannonIntervalDecrease); // Increase cannon fire rate
      return false;
    }
    return true;
  });
...
function displayHighScores(ctx) {
  ctx.font = "20px Arial";
  ctx.fillStyle = "black";
  ctx.fillText("High Scores:", 10, 70);
  highScores.forEach((entry, index) => {
    ctx.fillText(`${index + 1}. ${entry.name}: ${entry.score}`, 10, 90 + index * 20);
  });
}
